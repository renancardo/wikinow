...
function shouldUseFetchStreaming(): boolean {
...
async function* connectViaFetch(signal?: AbortSignal): AsyncGenerator<StreamRecentChangeEvent> {
  ...
  try {
    while (isActive()) {
      ...
      const events = parsePayloads(payloads, rawEventCountRef, isActive);
...
async function* connectViaXhr(signal?: AbortSignal): AsyncGenerator<StreamRecentChangeEvent> {
  const url = buildStreamUrl();
  ...
  const processChunk = (chunk: string) => {
    ...
    enqueue(parsePayloads(payloads, rawEventCountRef, isActive));
  };
