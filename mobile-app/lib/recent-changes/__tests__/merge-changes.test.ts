...
describe('mergeChanges', () => {
  it('returns incoming sorted when existing is empty', () => {
    ...
    expect(mergeChanges([], incoming, 'append')).toEqual([change(3), change(2), change(1)]);
  });

  it('returns existing sorted when incoming is empty', () => {
    const existing = [change(10), change(8)];

    expect(mergeChanges(existing, [], 'prepend')).toEqual([change(10), change(8)]);
  });

  it('dedupes by rcid and keeps the later occurrence in append mode', () => {
    const existing = [change(5, 'Old title'), change(4)];
    const incoming = [change(5, 'Updated title'), change(3)];

    const result = mergeChanges(existing, incoming, 'append');

    expect(result).toEqual([change(5, 'Updated title'), change(4), change(3)]);
  });

  it('dedupes by rcid and keeps the later occurrence in prepend mode', () => {
    const existing = [change(5, 'Stale title'), change(4)];
    const incoming = [change(6), change(5, 'Fresh title')];

    const result = mergeChanges(existing, incoming, 'prepend');

    expect(result).toEqual([change(6), change(5, 'Fresh title'), change(4)]);
  });

  it('prepends newer head items ahead of the existing tail', () => {
  ...
  it('appends older pagination items after the existing head', () => {
  ...
  it('handles overlapping refresh and pagination without duplicate rcids', () => {
    ...
    const afterPage1 = mergeChanges([], page1, 'append');
    const afterPage2 = mergeChanges(afterPage1, page2, 'append');
...
describe('mergeChanges head refresh scenario', () => {
  it('merges a new head page without losing paginated tail items', () => {
    ...
    const result = mergeChanges(tail, refreshedHead, 'prepend');
